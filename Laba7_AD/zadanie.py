...
# и разница между обучающей и тестовой выборкой всего 0.002,что очень мало
#Для Decision Tree все тоже неплохо,однако уступает по F-мере и разница в выборках 0.033
#
#KNN:
#1.Precision = 0.898 - это означает, что когда модель говорит "квартира будет продана", она права в 89.8% случаев
#2.Recall = 0.760 - означает, что модель находит 76% всех фактически проданных квартир
#3.Высокий Precision, умеренный Recall - модель редко ошибается, когда предсказывает продажу, но пропускает около 24% реальных продаж
#Decision tree:
#1.Precision = 0.910 - даже выше чем у KNN (91% точности положительных прогнозов)
#2.Recall = 0.731 - ниже чем у KNN (находит только 73.1% реальных продаж)
#3.Очень высокий Precision, низкий Recall - модель очень осторожна, минимизирует ложные срабатывания, но пропускает больше реальных продаж
